# 📋 Labuan FSA E-Submission System - Requirements Specification
...
- **Functional Requirements**: 92 (expanded from 52)
- **Non-Functional Requirements**: 25
- **Technical Requirements**: 10

**Note**: Functional requirements expanded to include comprehensive field type inventory (see `form-field-types-20251117-153458.md` for complete list of 85+ field types).
...
### FR-026 to FR-080: Frontend Component Requirements
...
### FR-041 to FR-080: Comprehensive Field Type Requirements

**Basic Input Types**:
| ID | Requirement | Priority | Effort |
|----|-------------|----------|--------|
| **FR-041** | System MUST support disabled and readonly input states | Must | 4h |
| **FR-042** | System MUST provide RichTextEditor component for WYSIWYG content | Should | 12h |
| **FR-043** | System MUST provide MarkdownEditor component for markdown content | Should | 8h |
| **FR-044** | System MUST support hidden input fields | Must | 2h |

**Selection Types**:
| ID | Requirement | Priority | Effort |
|----|-------------|----------|--------|
| **FR-045** | System MUST provide SelectWithOther component (dropdown with "Other" option triggering text input) | Must | 8h |
| **FR-046** | System MUST provide RadioWithOther component (radio group with "Other" option) | Must | 8h |
| **FR-047** | System MUST provide CheckboxWithOther component (checkbox group with "Other" option) | Must | 8h |
| **FR-048** | System MUST provide AsyncSelect component (options loaded from API on demand) | Should | 12h |
| **FR-049** | System MUST provide SearchableSelect component (search within options) | Must | 8h |
| **FR-050** | System MUST provide GroupedSelect component (options organized in groups) | Should | 8h |
| **FR-051** | System MUST provide CascadingSelect component (dependent dropdowns) | Should | 12h |
| **FR-052** | System MUST provide TagSelect component (tag/chip selection) | Should | 8h |
| **FR-053** | System MUST provide SegmentedControl component (iOS-style segmented buttons) | Should | 6h |

**File Upload Types**:
| ID | Requirement | Priority | Effort |
|----|-------------|----------|--------|
| **FR-054** | System MUST provide ImageUpload component with preview, crop, and resize | Must | 16h |
| **FR-055** | System MUST provide DocumentUpload component (PDF, DOC, DOCX with preview) | Must | 12h |
| **FR-056** | System MUST provide CameraCapture component (direct camera capture for mobile) | Should | 12h |
| **FR-057** | System MUST support chunked file upload for large files | Should | 16h |
| **FR-058** | System MUST support cloud storage direct upload (S3, Azure, GCP) | Should | 20h |

**Date & Time Types**:
| ID | Requirement | Priority | Effort |
|----|-------------|----------|--------|
| **FR-059** | System MUST provide DateRangePicker component (select date range) | Must | 12h |
| **FR-060** | System MUST provide MonthPicker component | Should | 6h |
| **FR-061** | System MUST provide YearPicker component | Should | 6h |
| **FR-062** | System MUST provide QuarterPicker component | Should | 6h |
| **FR-063** | System MUST provide WeekPicker component | Should | 6h |
| **FR-064** | System MUST provide TimeRangePicker component | Should | 8h |

**Payment Types**:
| ID | Requirement | Priority | Effort |
|----|-------------|----------|--------|
| **FR-065** | System MUST provide PaymentField component supporting Stripe integration | Should | 24h |
| **FR-066** | System MUST provide PaymentField component supporting PayPal integration | Should | 24h |
| **FR-067** | System MUST provide CardPayment component (credit card form) | Should | 20h |
| **FR-068** | System MUST provide PaymentSummary component (display payment details) | Should | 8h |

**Complex/Composite Types**:
| ID | Requirement | Priority | Effort |
|----|-------------|----------|--------|
| **FR-069** | System MUST provide ArrayField component (dynamic array of fields with add/remove) | Must | 20h |
| **FR-070** | System MUST provide TableField component (editable table with rows/columns) | Should | 24h |
| **FR-071** | System MUST provide DataGrid component (advanced grid with sorting, filtering) | Should | 32h |
| **FR-072** | System MUST provide ObjectField component (nested object structure) | Must | 16h |
| **FR-073** | System MUST provide NestedForm component (form within form) | Should | 20h |
| **FR-074** | System MUST provide TabsField component (tabbed field groups) | Should | 12h |
| **FR-075** | System MUST provide AccordionField component (collapsible field groups) | Should | 10h |
| **FR-076** | System MUST provide AddressField component (complete address input) | Should | 12h |
| **FR-077** | System MUST provide PhoneField component (international phone with country code) | Should | 12h |
| **FR-078** | System MUST provide CurrencyField component (currency input with formatting) | Should | 8h |
| **FR-079** | System MUST provide RangeSlider component (numeric range selection) | Should | 8h |
| **FR-080** | System MUST provide SignatureField component (digital signature capture) | Should | 16h |

**Note**: See `form-field-types-20251117-153458.md` for complete inventory of 85+ field types with detailed specifications.

### FR-081 to FR-092: Admin Dashboard Requirements

| ID | Requirement | Priority | Effort |
|----|-------------|----------|--------|
| **FR-081** | System MUST provide admin dashboard to view all forms and their schemas | Must | 12h |
| **FR-082** | System MUST provide admin dashboard to view all form submissions with filtering, sorting, and pagination | Must | 16h |
| **FR-083** | System MUST allow admin to review submissions (approve, reject, add notes) | Must | 12h |
| **FR-084** | System MUST allow admin to manage form schemas (CRUD operations) | Must | 16h |
| **FR-085** | System MUST provide export functionality for submission data (CSV, Excel, PDF) | Should | 12h |
| **FR-086** | System MUST implement user authentication and authorization for admin dashboard | Must | 12h |
| **FR-087** | System MUST provide audit logs for all admin actions | Must | 8h |
| **FR-088** | System MUST provide GET `/api/admin/submissions` endpoint with filtering and pagination | Must | 8h |
| **FR-089** | System MUST provide PUT `/api/admin/submissions/{id}` endpoint for submission review | Must | 8h |
| **FR-090** | System MUST provide GET `/api/admin/forms` endpoint for form management | Must | 4h |
| **FR-091** | System MUST support role-based access control for admin features | Must | 12h |
| **FR-092** | System MUST track submission review history (reviewed_by, reviewed_at, review_notes) | Must | 8h |
